...
const SkillsCRUD: React.FC<SkillsCRUDProps> = ({ onChange }) => {
  ...
  return (
    <div>
        ...
        <div className='flex items-center mb-[10px] mt-[10px]'>
            <Input
                placeholder="Skill e.g. (ai book generation, api integration)"
                value={taskName}
                onChange={(e) => setTaskName(e.target.value)}
            />
            <Button type="button" onClick={handleAddTask} className="bg-slate-200 text-black hover:text-white ml-[10px]">Add Skill</Button>
        </div>
        <ul>
            {tasks.map(task => (
            <li key={task.id}>
